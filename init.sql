CREATE TABLE Rezyserowie(
    ...
    CONSTRAINT uk_rezyser UNIQUE KEY(imie, nazwisko, data_urodzenia)
);
...
CREATE TABLE Filmy(
    ...
    CONSTRAINT uk_film UNIQUE KEY(nazwa, rok_produkcji).
...
CREATE TABLE Seriale(
    ...
    CONSTRAINT uk_serial UNIQUE KEY(nazwa, liczba_odcinkow),
...
CREATE TABLE Aktorzy(
    ...
    CONSTRAINT uk_aktor UNIQUE KEY(imie, nazwisko, data_urodzenia)
);
...
CREATE TABLE Postacie_serialowe(
    ...
    CONSTRAINT fk_postac_s_serial FOREIGN KEY(id_serialu) REFERENCES Seriale(id_seriale),
    CONSTRAINT fk_postac_s_aktor FOREIGN KEY(id_aktora) REFERENCES Aktorzy(id_aktora)
);

-- gatunek, moze jako zwykly pojednyczny atrybut? jesli encje slabe sie licza?
-- film/serial wiele rezyserow??? ;-;
